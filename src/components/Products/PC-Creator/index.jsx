...
const Index = () => {
    ...
    useEffect(() => {
        ...
        fetchListOfPcCreatorCollecting(1)
            .then(result => {
    ...
    const handleSetPage = (count) => {
        setProduct()
        setCountPage(count)
        fetchSearchPcCreatorCollecting(searchData, count)
            .then(result => {
                setProduct(result.data)
            })
            .catch(error => {
        ...
        if (name === "category") {
            ...
            if (inputFocused) {
                const timeoutId = setTimeout(() => {
                    fetchSearchPcCreatorCollecting(newSearchData, countPage)
                        .then(result => {
                            setProduct(result.data)
                            if (0 < result.total % 10 && result.total % 10 < 10) {
                                setCountMaxPage(Math.floor(result.total / 10) + 1)
    ...
    const handleOptionSelectedSecond = (e) => {
        ...
        if (inputFocused) {
            const timeoutId = setTimeout(() => {
                fetchSearchPcCreatorCollecting(newSearchData, 1)
                    .then(result => {
                        setProduct(result.data)
                        if (0 < result.total % 10 && result.total % 10 < 10) {
                            setCountMaxPage(Math.floor(result.total / 10) + 1)
    ...
    const handleSort = () => {
        ...
        fetchSearchPcCreatorCollecting({ ...searchData, sort: searchData.sort === 'asc' ? 'desc' : 'asc' }, 1)
            .then((result) => {
                setProduct(result.data);
                if (0 < result.total % 10 && result.total % 10 < 10) {
                    setCountMaxPage(Math.floor(result.total / 10) + 1);
