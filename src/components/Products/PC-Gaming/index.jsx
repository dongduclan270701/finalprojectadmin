...
const Index = () => {
    ...
    useEffect(() => {
        fetchCollectingByName("Pc Gaming")
            .then(result => {
        ...
        fetchListOfPcGamingCollecting(1)
            .then(result => {
    ...
    const handleSetPage = (count) => {
        setProduct()
        setCountPage(count)
        fetchSearchPcGamingCollecting(searchData, count)
            .then(result => {
                setProduct(result.data)
            })
            .catch(error => {
        ...
        if (name === "category") {
            ...
            if (inputFocused) {
                const timeoutId = setTimeout(() => {
                    fetchSearchPcGamingCollecting(newSearchData, countPage)
                        .then(result => {
                            setProduct(result.data)
                            if (0 < result.total % 10 && result.total % 10 < 10) {
                                setCountMaxPage(Math.floor(result.total / 10) + 1)
    ...
    const handleOptionSelectedSecond = (e) => {
        ...
        if (inputFocused) {
            const timeoutId = setTimeout(() => {
                fetchSearchPcGamingCollecting(newSearchData, 1)
                    .then(result => {
                        setProduct(result.data)
                        if (0 < result.total % 10 && result.total % 10 < 10) {
                            setCountMaxPage(Math.floor(result.total / 10) + 1)
    ...
    const handleSort = () => {
        ...
        fetchSearchPcGamingCollecting({ ...searchData, sort: searchData.sort === 'asc' ? 'desc' : 'asc' }, 1)
            .then((result) => {
                setProduct(result.data);
                if (0 < result.total % 10 && result.total % 10 < 10) {
                    setCountMaxPage(Math.floor(result.total / 10) + 1);
                        ...
                        <div className="col-lg-12 grid-margin stretch-card">
                            <div className="card">
                                <div className="card-body">
                                    <h4 className="card-title">List of Pc Gaming products</h4>
                                    <NavLink to={"/pc-gaming/create"} className="card-description" style={{ textDecoration: "none" }}>
                                                    ...
                                                    <tbody>
                                                        {product.map((item, index) => {
                                                            return <tr key={index}>
                                                                ...
                                                                <td>
                                                                    <NavLink to={"/pc-gaming/" + item.src} ><button type="button" className="btn btn-outline-secondary btn-fw">Show</button></NavLink>
                                                                </td>
                                                            </tr>
                                                        })}
                                                    </tbody>
